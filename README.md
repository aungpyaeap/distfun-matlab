...
| Distance Name | Formula | Data Type | Range | Origin | Ref. |
|----------|----------|----------|----------|----------|----------|
| Euclidean distance         |$d(x_i,x_j) = \sqrt{\sum_{k=1}^m (x_{ik} - x_{jk})^2}$| Numerical          | $[0, +\infty\rangle$ | $d$ | [2][7]|
| Squared Euclidean distance |$d(x_i,x_j) = \sum_{k=1}^m (x_{ik} - x_{jk})^2$| Numerical          | $[0, +\infty\rangle$ | $d$ | [2][7]|
| City block distance        |$d(x_i,x_j) = \sum_{k=1}^m \vert x_{ik} - x_{jk}\vert$| Numerical          | $[0, +\infty\rangle$ | $d$ | [2][7]|
| Chebyshev distance         |$d(x_i,x_j) = \max_{k=1}^m \vert x_{ik} - x_{jk}\vert$| Numerical          | $[0, +\infty\rangle$ | $d$ | [2][7]|
| Canberra distance          |$d(x_i,x_j) = \sum_{k=1}^{m} \frac{\vert x_{ik} - x_{jk}\vert}{\vert x_{ik}| + |x_{jk} \vert}$| Numerical          | $[0, +\infty\rangle$ | $d$ | [2]|
